...
execute at @e[type=marker,limit=1,sort=nearest,tag=DragonChargeRotation] rotated ~20 0 run summon marker ^ ^ ^-1 {Tags:["SmoothTurn","Right"]}
...
execute at @e[type=marker,limit=1,sort=nearest,tag=DragonChargeRotation] rotated ~-20 0 run summon marker ^ ^ ^-1 {Tags:["SmoothTurn","Left"]}
...
execute if entity @e[type=marker,tag=SmoothTurn,tag=Closest,tag=Right] at @s run scoreboard players add @s RVel 90
