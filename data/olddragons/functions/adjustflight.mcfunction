...
#Fix an issue where knockback from explosions sends the ender dragon extremely high
execute unless data entity @s {HurtTime:0s} store result score YVel Temp run data get entity @s Motion[1] 1000
execute unless data entity @s {HurtTime:0s} if score YVel Temp > YVelUB Settings run scoreboard players operation YVel Temp = YVelUB Settings
execute unless data entity @s {HurtTime:0s} if score YVel Temp < YVelLB Settings run scoreboard players operation YVel Temp = YVelLB Settings
execute unless data entity @s {HurtTime:0s} store result entity @s Motion[1] double 0.001 run scoreboard players get YVel Temp
#Calculate movement
