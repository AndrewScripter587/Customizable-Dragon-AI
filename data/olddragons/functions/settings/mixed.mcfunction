#Allow both the vanilla behavior and the mechanics added in this datapack to coexist
scoreboard players set Allow1.9Behavior Settings 1
scoreboard players set ReplaceVanillaCharging Settings 0
scoreboard players set MiscDragonTweaks Settings 1
scoreboard players set MorePerching Settings 1
scoreboard players set ChargeRate Settings 5
scoreboard players set DragonCharging Settings 1
scoreboard players set BedrockStyle Settings 1
scoreboard players set FlySpeed Settings 1900
scoreboard players set ChargeFlySpeed Settings 200
scoreboard players set VerticalSmoothing Settings 40
scoreboard players set HorizontalSmoothing Settings 40
scoreboard players set Slowdown Settings 15
scoreboard players set MimicVanillaCharging Settings 0
